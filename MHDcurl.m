function [ curlR, curlPHI, curlZ ] = MHDcurl( Rcomp, PHIcomp, Zcomp, r, z  )
...
%GP
dPdz(row-1,:) = ((1/12)*P(row-1,:)+(-2/3)*P(row-1,:) +(2/3)* P(row-2,:)+(-1/12)* P(row-3,:))/dz;
...
%GP          
zinEE(:,col-1) = (-1/r(col-1)) *((1/12)*rphi(:,col-1)+(-2/3)*rphi(:,col-1) ...
...
curlZ(:,col)=zinEE(:,col);
%GP
dpdz(2,:) = ((1/12)*P(2,:)+(-2/3)*P(2,:) +(2/3)* P(3,:)-(1/12)* P(4,:))/dz;
...
%GP
 zinEE(:,2) = 1/r(2) *  ((1/12)*rphi(:,2)-(2/3)*rphi(:,2)+(2/3)*rphi(:,3)-(1/12)*rphi(:,4)) / dr;
...
zinEE(:,2) = -(-1/r(2)) *((1/12)*rphi(:,2)+(-2/3)*rphi(:,2) ...
                   +(2/3)* rphi(:,3)-(1/12)* rphi(:,4))/dr;
zinEE(:,1) = -(-1/r(1)) *((1/12)*rphi(:,1)+(-2/3)*rphi(:,1) ...
...
%GP
drdz1(row-1,:)= ((1/12)*R(row-1,:) +(-2/3)*R(row-1,:)+(2/3)*R(row-2,:) +(-1/12)*R(row-3,:) )/dz;
drdz1(row,:)= ((1/12)*R(row,:) +(-2/3)*R(row,:)+(2/3)*R(row-1,:) +(-1/12)*R(row-2,:) )/dz;
% dzdr1(:,col)=(Z(:,col)-Z(:,col-1))/dr;
% dzdr1(:,col-1)=(Z(:,col)-Z(:,col-2))/2/dr; 
%GP
dzdr1(:,col-1)=((1/12)*Z(:,col-1)+(-2/3)*Z(:,col-1)+(2/3)*Z(:,col-2)+(-1/12)*Z(:,col-3))/dr; 
...
%GP
curlR(2,2) = ((1/12)*P(2,2)+(-2/3)*P(2,2) +(2/3)* P(3,2)+(-1/12)* P(4,2))/dz;
curlR(2,1) = ((1/12)*P(2,1)+(-2/3)*P(2,1) +(2/3)* P(3,1)+(-1/12)* P(4,1))/dz;
...
%GP
dzindr(2,2) = ((1/12)*rphi(2,2)+(-2/3)*rphi(2,2) +(2/3)* rphi(2,3)+(-1/12)* rphi(2,4))/dr;
dzindr(2,1) = ((1/12)*rphi(2,1)+(-2/3)*rphi(2,1) +(2/3)* rphi(2,2)+(-1/12)* rphi(2,3))/dr;
dzindr(1,2) = ((1/12)*rphi(1,2)+(-2/3)*rphi(1,2) +(2/3)* rphi(1,3)+(-1/12)* rphi(1,4))/dr;
...
%GP
dzdr(2,2)=((1/12)*Z(2,2)+(-2/3)*Z(2,2) +(2/3)* Z(2,3)+(-1/12)*Z(2,4))/dr;
dzdr(2,1)=((1/12)*Z(2,1)+(-2/3)*Z(2,1) +(2/3)* Z(2,2)+(-1/12)*Z(2,3))/dr;
dzdr(1,2)=((1/12)*Z(1,2)+(-2/3)*Z(1,2) +(2/3)* Z(1,3)+(-1/12)*Z(1,4))/dr;
...
%GP
drdz(2,2)=((1/12)*R(2,2)+(-2/3)*R(2,2)+ (2/3)* R(3,2)+(-1/12)* R(4,2))/dz;
drdz(2,1)=((1/12)*R(2,1)+(-2/3)*R(2,1)+ (2/3)* R(3,1)+(-1/12)* R(4,1))/dz;
...
%GP
curlR(2,col-1)=-((1/12)*P(2,col-1)+(-2/3)*P(2,col-1) +(2/3)* P(3,col-1)+(-1/12)* P(4,col-1))/dz;
curlR(2,col)=-((1/12)*P(2,col)+(-2/3)*P(2,col) +(2/3)* P(3,col)+(-1/12)* P(4,col))/dz;
...
%GP
dzindr(2,col-1) =-((1/12)*rphi(2,col-1)+(-2/3)*rphi(2,col-1) +(2/3)* rphi(2,col-2)+(-1/12)* rphi(2,col-3))/dr;
dzindr(2,col) =-((1/12)*rphi(2,col)+(-2/3)*rphi(2,col) +(2/3)* rphi(2,col-1)+(-1/12)* rphi(2,col-2))/dr;
dzindr(1,col-1) =-((1/12)*rphi(1,col-1)+(-2/3)*rphi(1,col-1) +(2/3)* rphi(1,col-2)+(-1/12)* rphi(1,col-3))/dr;
...
%GP
dzdr(2,col-1)=-((1/12)*Z(2,col-1)+(-2/3)*Z(2,col-1) +(2/3)* Z(2,col-2)+(-1/12)*Z(2,col-3))/dr;
dzdr(2,col)=-((1/12)*Z(2,col)+(-2/3)*Z(2,col) +(2/3)* Z(2,col-1)+(-1/12)*Z(2,col-2))/dr;
dzdr(1,col-1)=-((1/12)*Z(1,col-1)+(-2/3)*Z(1,col-1) +(2/3)* Z(1,col-2)+(-1/12)*Z(1,col-3))/dr;
...
%GP
drdz(2,col-1)=((1/12)*R(2,col-1)+(-2/3)*R(2,col-1)+(2/3)* R(3,col-1)+(-1/12)* R(4,col-1))/dz;
drdz(2,col)=((1/12)*R(2,col)+(-2/3)*R(2,col)+(2/3)* R(3,col)+(-1/12)* R(4,col))/dz;
...
%                       P(row-2,col)+(1/3)* P(row-3,col))/dz;                  
%GP
curlR(row-1,col-1)=((1/12)*P(row-1,col-1)+(-2/3)*P(row-1,col-1) +(2/3)*P(row-2,col-1)+(-1/12)* P(row-3,col-1))/dz;
curlR(row-1,col)=((1/12)*P(row-1,col)+(-2/3)*P(row-1,col) +(2/3)*P(row-2,col)+(-1/12)* P(row-3,col))/dz;
...
%GP                 
dzindr(row-1,col-1)=-((1/12)*rphi(row-1,col-1)+(-2/3)*rphi(row-1,col-1)+(2/3)*rphi(row-1,col-2)+(-1/12)*rphi(row-1,col-3))/dr;
dzindr(row-1,col)=-((1/12)*rphi(row-1,col)+(-2/3)*rphi(row-1,col)+(2/3)*rphi(row-1,col-1)+(-1/12)*rphi(row-1,col-2))/dr;
dzindr(row,col-1)=-((1/12)*rphi(row,col-1)+(-2/3)*rphi(row,col-1)+(2/3)*rphi(row,col-2)+(-1/12)*rphi(row,col-3))/dr;
...
%GP
dzdr(row-1,col-1)=-((1/12)*Z(row-1,col-1)+(-2/3)*Z(row-1,col-1)+(2/3)* Z(row-1,col-2)+(-1/12)*Z(row-1,col-3))/dr;
dzdr(row-1,col)=-((1/12)*Z(row-1,col)+(-2/3)*Z(row-1,col)+(2/3)* Z(row-1,col-1)+(-1/12)*Z(row-1,col-2))/dr;
dzdr(row,col-1)=-((1/12)*Z(row,col-1)+(-2/3)*Z(row,col-1)+(2/3)* Z(row,col-2)+(-1/12)*Z(row,col-3))/dr;
...
%GP
drdz(row-1,col-1)=-((1/12)*R(row-1,col-1)+(-2/3)*R(row-1,col-1)+(2/3)* R(row-2,col-1)+(-1/12)* R(row-3,col-1))/dz;
drdz(row-1,col)=-((1/12)*R(row-1,col)+(-2/3)*R(row-1,col)+(2/3)* R(row-2,col)+(-1/12)* R(row-3,col))/dz;
...
%                       P(row-2,1)+(1/3)* P(row-3,1))/dz; 
%GP
curlR(row-1,2) = ((1/12)*P(row-1,2)+(-2/3)*P(row-1,2) +(2/3)*P(row-2,2)+(-1/12)* P(row-3,2))/dz;
curlR(row-1,1) = ((1/12)*P(row-1,1)+(-2/3)*P(row-1,1) +(2/3)*P(row-2,1)+(-1/12)* P(row-3,1))/dz;
...
%GP                 
dzindr(row-1,2)  =((1/12)*rphi(row-1,2)+(-2/3)*rphi(row-1,2)+(2/3)*rphi(row-1,3)+(-1/12)*rphi(row-1,4))/dr;
dzindr(row-1,1)  =((1/12)*rphi(row-1,1)+(-2/3)*rphi(row-1,1)+(2/3)*rphi(row-1,2)+(-1/12)*rphi(row-1,3))/dr;
dzindr(row,2)  =((1/12)*rphi(row,2)+(-2/3)*rphi(row,2)+(2/3)*rphi(row,3)+(-1/12)*rphi(row,4))/dr;
...
%GP
dzdr(row-1,2)=((1/12)*Z(row-1,2)+(-2/3)*Z(row-1,2)+(2/3)*Z(row-1,3)+(-1/12)*Z(row-1,4))/dr;
dzdr(row-1,1)=((1/12)*Z(row-1,1)+(-2/3)*Z(row-1,1)+(2/3)*Z(row-1,2)+(-1/12)*Z(row-1,3))/dr;
dzdr(row,2)=((1/12)*Z(row,2)+(-2/3)*Z(row,2)+(2/3)*Z(row,3)+(-1/12)*Z(row,4))/dr;
...
%GP
drdz(row-1,2)=-((1/12)*R(row-1,2)+(-2/3)*R(row-1,2)+(2/3)* R(row-2,2)+(-1/12)* R(row-3,2))/dz;
drdz(row-1,1)=-((1/12)*R(row-1,1)+(-2/3)*R(row-1,1)+(2/3)* R(row-2,1)+(-1/12)* R(row-3,1))/dz;
